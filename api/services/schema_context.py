...
def build_system_prompt(schema: dict[str, dict[str, list[str]]], layer: str = "warehouse") -> str:
    ...
    lines.append("- For service location/address data, PREFER public_warehouse.dim_service_location (columns: address, city, state, zip, rate, rate_type, labor_cost). Fall back to public_staging.stg_service_location only if dim_service_location is missing a needed column")
    ...
    lines.append("- **CANONICAL active filter (vendor-authoritative per Glenn Burnside, Skimmer VP Eng, 2026-04-22):**")
    lines.append("  WHERE ra.end_date > CURRENT_TIMESTAMP::text AND c.deleted = 0 AND sl.deleted = 0")
    lines.append("- **DO NOT** add a service-history filter (e.g. 'serviced in last 90 days'). Per Laura Gergle, this incorrectly excludes new customers who have been added to a route but not yet had their first service visit.")
    lines.append("- Sentinel: EndDate = '2080-01-01 12:00:00' is Skimmer's 'no end date' marker (~55 years in the future, vendor convention). This IS current — 3,154 rows carry it. Note the NOON time component; filtering on '2080-01-01 00:00:00' misses everything.")
    ...
    lines.append("- There is NO dim_route_assignment dim — route analytics use public_semantic.rpt_active_routes (materialized nightly from stg_route_assignment + dim_tech + dim_service_location + dim_customer) OR stg_route_assignment directly.")
    lines.append("- When presenting 'never serviced' routes, always label them observationally (e.g. 'awaiting first service') — NEVER as 'stale' or 're-routing candidate'. They are most often new customers pending first visit.")

...
**⚠️ Issue #4 — host-level Python/dbt were NOT migrated (RUNBOOK GAP, found 2026-06-27, fixed 2026-07-04):** the migration carried the repo + Docker containers but not the **host-installed** toolchain the nightly pipeline runs outside Docker: `psycopg2` (ETL), `dbt` (transform), and `dbt_packages/` (gitignored, needs `dbt deps`). Result: the nightly pipeline failed silently-ish every night post-cutover — first `ModuleNotFoundError: psycopg2`, then `dbt: command not found` (exit 127), then missing dbt deps (exit 2). Raw loads/replaces continued but the warehouse layer went stale, and no `reconciliation.json` was produced. **FIX:** provision a project venv (`python3 -m venv .venv && .venv/bin/pip install -r etl/requirements.txt dbt-postgres==1.9.1` — pin stable; bare `pip install dbt-postgres` resolves a **beta** dbt-core) + `dbt deps`, and `nightly-pipeline.sh` now auto-activates `.venv` when present. **Make "inventory host-level (non-Docker) runtime deps" a standard migration step** — anything cron runs directly on the host is invisible to a container-centric migration. Corollary lesson: **verify the first full nightly after any migration**, not just service liveness — this gap rode along undetected for 9 nights.

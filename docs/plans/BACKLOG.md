...
| ~~IN-22~~ | ~~**Restricted archive destination for BBSI payroll/HR drop-off**~~ | ~~Security/Infra~~ | ~~S~~ | ~~**DONE 2026-09-16.** `infrastructure/sftp/archive-restricted.sh` — copies PII-bearing drop-offs to `/opt/splashworks/data/restricted/<account>/` at **0700 dirs / 0600 files** (tighter than the shared archive's 0750/0640), cron hourly at :17. Could not reuse `archive-incoming.sh`: it copies only payloads whose `*.manifest.json` verifies and BBSI sends none, so a retargeted script would archive nothing and exit 0. This one carries its own completion test — a partner checksum sidecar (`.sha256`/`.sha512`/`.md5`) when present, else settle-time (mtime ≥ SETTLE_MIN **and** size stable across two reads) — and writes a `<file>.receipt.json` recording sha256, bytes, source mtime and **`verified` true/false with the method used**, so a heuristic copy is never mistaken for a verified one. Never deletes from the drop-off. Retention mechanism built but **report-only by default** (`RETENTION_MODE=delete` to arm) — scheduled deletion of payroll PII is Ross's policy call. Optional gpg encryption via `RESTRICTED_GPG_RECIPIENT`; unset means plaintext at 0600 and the log says so every run. 13-arm fixture test incl. checksum-mismatch-refused and settle-gate arms. Verified on the real first upload 2026-09-16 14:55. **Follow-ups carded: IN-23 encryption at rest, IN-24 retention sign-off.**~~
| IN-23 | **Encryption at rest for the restricted archive** | Security | S | `archive-restricted.sh` supports `RESTRICTED_GPG_RECIPIENT` today but is running **unencrypted at 0600**. The blocker is key management, not code: a private key stored on the same box protects against nothing. Needs a gpg keypair whose private half lives in Bitwarden and never touches the VPS, plus a documented restore path — an archive nobody can decrypt is worse than plaintext. Closes runbook OPEN item 5. |
| IN-24 | **Retention policy sign-off for payroll/HR PII** | Governance | S | Mechanism is built and defaults to `RETENTION_MODE=report` — it names what it would delete and deletes nothing. Needs an actual policy: how long Splashworks should hold BBSI payroll data, agreed with BBSI as PEO. Precedent is IN-12 (redact @90d, delete @365d) for audit logs. Until this is set, restricted copies accumulate indefinitely. |
